...
- **Browser bugs:** Prefer the active Codex Browser/browser-use tools when available. Otherwise use whatever works — browser MCP tools, Playwright, direct URL testing, screenshot capture, etc.
...
3. **Rethink the design** — only when the root cause reveals a design problem (see below). For rolling-wave projects, route to `shape-project` for project-level design or `prepare-next-slice` for slice-level design.
...
|---------|-----------|-----------|
| Hypotheses point to different subsystems | Architecture/design problem, not a localized bug | Present findings, suggest `shape-project` or `prepare-next-slice` depending on scope |
...
If the user chose "Diagnosis only" at the end of Phase 2, skip this phase and go straight to Phase 4 for the summary — the skill's job was the diagnosis. If they chose "Rethink the design", stop after the summary and tell the user which rolling-wave shaping/preparation skill should pick it up.
...
6. Self-review the diff before declaring the fix done: read every changed line and check for style violations, missed edge cases, regressions in adjacent behavior, and missing test coverage for the fix. For non-trivial fixes (multiple files, risky surface area), also run the harness's lightweight review tool or `code-review` if available.
...
1. **Review the fix** — run the harness's lightweight review tool or `code-review` if available
2. **Commit / open a PR** — use the user's normal commit or PR workflow
3. **Stop here** — user takes it from there

#### After a fix is complete: consider offering learning capture
...
When offering, use the blocking question tool described above. If the user accepts, write the learning into the project's durable learning location if one exists, or into the active rolling-wave project/slice notes when this bug belongs to a rolling-wave slice.
